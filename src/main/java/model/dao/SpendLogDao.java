...
public class SpendLogDao {
	...
	public List<SpendLog> findByUserId(String userId) throws ClassNotFoundException {
		...
		try (Connection conn = DriverManager.getConnection("jdbc:oracle:thin:@3.34.136.55:1521:xe", "cashflow",
				"oracle")) {
			String sql = "SELECT s.*, c.NAME FROM SPEND_LOGS s JOIN CATEGROYS c ON s.CATEGORY_ID = c.ID WHERE USER_ID=?";
			PreparedStatement pstmt = conn.prepareStatement(sql);

			ResultSet rs = pstmt.executeQuery();
			List<SpendLog> list = new ArrayList<>();
			while (rs.next()) {
				// SpendLog(int no, String userId, Date spendAt, int amt, String useDesc, int categoryId, Category category) {
				SpendLog log = new SpendLog(
							rs.getInt("no"), 
							rs.getString("user_id"), 
							rs.getDate("spend_at"), 
							rs.getInt("amt"), 
							rs.getString("use_desc"), 
